...
entry's relaxed `"max-lines"` cap and `"max-lines-per-function": "off"` — the top-level size rules
depend on them). Reconcile those rule keys with the same add/conflict logic as `rules`; treat only
project-added rules as owned.
