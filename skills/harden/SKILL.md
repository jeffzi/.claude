...
3. Confirm both finder skills are invocable: `bug-scan` and `ponytail:ponytail-review` (a plugin
   skill — it may not be installed here). If either is unavailable, STOP and name the missing one —
   a round without the correctness rubric or the simplification lens is incomplete, not lean.
...
1. **Audit fan-out.** Full scan by default: dispatch parallel read-only finder agents across every
   ...
   instructs the sub-agent to invoke `Skill(bug-scan)` (correctness rubric) or
   `Skill(ponytail:ponytail-review)` (simplification lens) and work from the live source. A pasted
   gloss forks the skill and goes stale the next time it changes. Dispatch with an agent type that
   can call `Skill` (e.g. `bug-scanner`) — on a Read/Glob/Grep-only finder the load silently no-ops.
...
| "Bug and DRY finders cover the fan-out — the ponytail lens is optional"                    | The simplification lens is mandatory and catches what correctness finders miss (stdlib / native / over-engineering). A roster without it is incomplete, not lean — STOP and add it.                                              |
| "I'll inline the finder's rubric in the prompt — same content, one fewer skill load"       | A skill is the source of truth; a pasted gloss forks it and goes stale the next time `bug-scan` or `ponytail-review` is edited. The finder prompt must tell the sub-agent to invoke the skill, not carry a copy of its contents. |
