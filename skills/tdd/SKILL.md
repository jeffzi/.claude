...
| "Plan task → proceed, so I commit and list the findings after" | The grant covers verified work, not unseen findings. SURFACE gate first: findings, end turn, then commit. Only an unattended plan execution (the injected policy line says so) lifts this — then findings go to the plan's Findings list and the loop continues. |
...
- **Surface before commit.** Any open item — unconfirmed finding, flaky test, "unrelated" issue — is
  shown to the user and the turn ends before any commit (orchestration-flow § SURFACE). Under
  unattended plan execution (the injected policy line says so) the items go to the plan's Findings
  list and the turn continues.
