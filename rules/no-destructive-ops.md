...
- **Git (discarding changes)**: `checkout` (all forms — use `git switch` for branches, `git restore
  --staged` for unstaging), `switch -f`/`--force`/`--discard-changes`, `restore` (without
  `--staged`), `reset` (all forms including `reset HEAD`), `clean`, `stash`, `apply -R`/`--reverse`.
  To unstage files, use `git restore --staged <path>` instead of `git reset`.
